...
export function localBusiness(siteUrl: string = SITE.url): Record<string, unknown> {
  ...
  const schema: Record<string, unknown> = {
    ...
    image: SITE.ogImage ? `${root}${SITE.ogImage}` : undefined,
  ...
  // Address — locality-level is correct for a service-area business; streetAddress optional.
  ...
  // Only emit review markup when it's real — Google disallows self-serving/fake aggregateRating.
